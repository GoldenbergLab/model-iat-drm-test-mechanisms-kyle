    ...
    real race_reduce(real[] RT, int start, int end, int[] choice, int[] N_ind, int[] N_sub_ind, int[] condition, real[] mu_alpha, real[] mu_v_lower, real[] mu_v_upper, real mu_tau, real[,] alpha, real[,] v_lower, real[,] v_upper, real[] tau){
        ...
        for (i in start:end) {
            ...
            if (N_sub_ind[N_ind[i]] != 0) {
                alpha_i = alpha[N_sub_ind[N_ind[i]], cond];
                v_lower_i = v_lower[N_sub_ind[N_ind[i]], cond];
                v_upper_i = v_upper[N_sub_ind[N_ind[i]], cond];
                tau_i = tau[N_sub_ind[N_ind[i]]];
            }
            else {
                alpha_i = mu_alpha[cond];
                v_lower_i = mu_v_lower[cond];
                v_upper_i = mu_v_upper[cond];
                tau_i = mu_tau;
            }
...
parameters {
    ...
    real<lower=0> mu_alpha[N_cond-1];
    real<lower=0.1> mu_tau;
    real<lower=0> mu_v_lower[N_cond-1];
    real<lower=0> mu_v_upper[N_cond-1];

    // Hyperparameter variances
    real<lower=0> sigma_alpha[N_cond-1];
    real<lower=0> sigma_tau;
    real<lower=0> sigma_v_lower[N_cond-1];
    real<lower=0> sigma_v_upper[N_cond-1];

    // Individual parameters
    real<lower=0> alpha[N_sub, N_cond-1];
	real<lower=0.1> tau[N_sub];
	real<lower=0> v_lower[N_sub, N_cond-1];
	real<lower=0> v_upper[N_sub, N_cond-1];
}
model {
    // Hyperparameter means
